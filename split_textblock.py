...
if __name__ == "__main__":
            ...
            try:
                    ...
                    if not result_df.empty:
                        ...
                        if '处理状态' in df.columns:
                            ...
                            if not problem_rows.empty:
                                ...
                                law_df.loc[original_index, '问题标签'] = '不是按照中文数字加顿号分类的'
                    ...
                    if pd.isnull(law_df.loc[row.name, '问题标签']) or law_df.loc[row.name, '问题标签'] == '':  # 只在无问题标签时生成
                        try:
                            ...
                            # 生成Excel文件部分添加异常捕获
                            if pd.isnull(law_df.loc[row.name, '问题标签']) or law_df.loc[row.name, '问题标签'] == '':
                                try:
                                    with open(error_log, 'a', encoding='utf-8') as f:
                                        f.write(f"{name}\n")
    ...
    if not error_df.empty:
        error_path = os.path.join(os.path.dirname(excel_path), "生成失败条目.xlsx")
        error_df.to_excel(error_path, index=False, engine='openpyxl')
        print(f"\n已保存 {len(error_df)} 条失败记录至：{error_path}")

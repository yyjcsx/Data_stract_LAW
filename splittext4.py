...
def legal_text_to_dataframe1(text: str) -> pd.DataFrame:
    ...
    for block in blocks:
        ...
        if volume_pattern.match(first_line):
            ...
            # 处理条目（拆分目录和内容）
            entry_header = entry_pattern.search(first_line).group(1).strip()
            # 使用split分割条目目录和内容
            entry_content = first_line.split(entry_header, 1)[-1].strip()
            if '\n' in block:
                entry_content += '\n' + '\n'.join(block.split('\n')[1:])
            ...
            result.append({
                ...
                "节索引": current_section["number"] if current_section["index"] > 0 else "",
                "节名称": current_section["name"] if current_section["index"] > 0 else "",
                "条目目录": entry_header,  # 仅保留"一、"
                "条目内容": entry_content.strip()  # 包含后续所有内容
            })
